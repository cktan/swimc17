...
static int pack_node_id(const swim_node_id_t *id, uint8_t *p, uint8_t *q) {
  uint8_t *start = p;
  int n;
...
int swim_unpack_message(const uint8_t *buf, size_t size, swim_message_t *msg) {
  ...
  n = unpack_node_id(&msg->sender, p, end);
  if (n < 0) {
  ...
  if (msg->type == SWIM_MSG_PING_REQ || msg->type == SWIM_MSG_FWD_ACK) {
    n = unpack_node_id(&msg->peer, p, end);
    if (n < 0) {
