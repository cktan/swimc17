...
// Compactly encode a Node ID: [host_len (1B)] [host (NB)] [port (2B)]
// [cookie_len (1B)] [cookie (MB)]
static int encode_node_id(const swim_node_id_t *id, uint8_t *buf, size_t limit,
                          size_t *offset) {
...
int swim_codec_encode(const swim_message_t *msg, uint8_t *buf, size_t size) {
  if (!msg || !buf || size == 0) {
    return swim_set_error(
        SWIM_ERR_INVALID,
        "Invalid NULL or zero size arguments to swim_codec_encode");
  }
...
int swim_codec_decode(const uint8_t *buf, size_t size, swim_message_t *msg) {
  ...
  for (int i = 0; i < msg->event_count; i++) {
    ...
    if (offset + 1 > size) {
      return swim_set_error(SWIM_ERR_INVALID,
                            "Buffer too short for event status");
    }
    ev->status = (swim_status_t)buf[offset++];
    if (ev->status != SWIM_STATUS_ALIVE && ev->status != SWIM_STATUS_SUSPECT &&
    ...
    if (offset + 8 > size) {
      return swim_set_error(SWIM_ERR_INVALID,
                            "Buffer too short for event incarnation");
    }
